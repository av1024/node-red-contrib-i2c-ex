    ...
    function i2cMCP23017cfg(cfg) {
      ...
      n.reconfig = (full) => {
        ...
        if (full) {
          ...
          n.port.writeI2cBlockSync(n.addr, 0x05, 1, [0]);  // reset IOCON.BANK in "1" mode  --> switch to "0" mode, reset rq if in "0" mode
          n.port.writeI2cBlockSync(n.addr, 0x0A, 1, [bbb]);  // reset IOCON.BANK in "0" mode
        }
        ...
        // set inverts
        // set pinirq mode
        n.port.writeI2cBlockSync(n.addr, 0x00, 10, [
          n.inBits<<8, n.inBits>>8,  //IODIR A/B
          n.invertBits<<8, n.invertBits>>8, // IOPOL A/B
          n.irqBits<<8, n.irqBits>>8,  // INTEN A/B
          0, 0, // DEFVAL (unused)
          0, 0 // INTCON: compare with prev value
        ]); // 0, 1
        
        // set pullups
        n.port.writeI2cBlockSync(n.addr, 0x0C, 2, [
          n.pullupBits<<8, n.pullupBits>>8,  // GPPU A/B
        ]);
      ...
      n.readIn = (force, only_bit) => {
        var v = 0x0000; // read i2c 16 bit here
        var iqv = 0x0000; // read IRQ register        
